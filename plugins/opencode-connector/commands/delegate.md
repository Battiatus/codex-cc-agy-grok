---
description: Delegate a bounded task to OpenCode, read-only by default
argument-hint: '[--mode write --confirm-write] [--model <model>] [--effort <level>] [--background] [--timeout 10m] <task>'
allowed-tools: Bash(node:*), AskUserQuestion
---

Delegate to OpenCode through the bundled bridge.

Raw arguments: `$ARGUMENTS`

Rules:
- Default to review mode. Pass `--mode write --confirm-write` only when the user explicitly asked OpenCode to change files in this workspace.
