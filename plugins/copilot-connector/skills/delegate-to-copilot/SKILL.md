...
description: Delegate a bounded review or coding task to Copilot through the local gh CLI, with a git-scoped diff and a schema-validated verdict. Use for independent review, second diagnosis, cross-provider handoff, status, results or cancellation.
---

# VSCode Copilot Connector

Always call the bundled bridge. Never construct `gh` command lines yourself.
...
- The bridge resolves the git scope itself and embeds the exact diff in the prompt, so Copilot sees the change even without shell or git tools.
...
- Credentials are never read or stored; the bridge inherits the Copilot login.
