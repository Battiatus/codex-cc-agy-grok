---
description: Run an adversarial Red Team code review with Copilot focusing on security vulnerabilities, edge cases, and exploit vectors
...
Delegate an adversarial Red Team review to Copilot through the bundled bridge.
...
- If the arguments include `--background`, launch the command with `run_in_background: true` and tell the user to check `/copilot-connector:status`.
...
- `COULD_NOT_REVIEW` means Copilot could not assess the change. Report why and stop.
- `SCHEMA_VIOLATION` means the payload was malformed. Report the schema errors and stop.
- `COMPLETED_WITH_DENIALS` means Copilot was blocked from part of the work. List the denials.
