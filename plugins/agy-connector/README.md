...
Delegate bounded, git-scoped review and coding tasks to the local Antigravity CLI from Codex, Claude Code, Grok Build, or Antigravity.
...
- Target executable: `agy`, resolved through a native binary, an npm shim entrypoint or a shebang, so Windows `.cmd` shims work
- Default mode: `review` — read-only, enforced by plan-mode, run in place so git history stays available
...
- Routing: `--model`, `--effort` (low, medium, high), `--stream`
- State: `AGENT_CONNECTOR_HOME`, or the OS temporary directory under `agent-connectors/agy`
