---
description: Run an adversarial Red Team code review with Claude Code focusing on security vulnerabilities, edge cases, and exploit vectors
...
Delegate an adversarial Red Team review to Claude Code through the bundled bridge.
...
- If the arguments include `--background`, launch the command with `run_in_background: true` and tell the user to check `/claude-connector:status`.
...
- `COULD_NOT_REVIEW` means Claude Code could not assess the change. Report why and stop.
- `SCHEMA_VIOLATION` means the payload was malformed. Report the schema errors and stop.
- `COMPLETED_WITH_DENIALS` means Claude Code was blocked from part of the work. List the denials.
