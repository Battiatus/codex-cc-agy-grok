...
Delegate bounded, git-scoped review and coding tasks to the local Codex CLI from Codex, Claude Code, Grok Build, or Antigravity.
...
- Target executable: `codex`, resolved through a native binary, an npm shim entrypoint or a shebang, so Windows `.cmd` shims work
- Default mode: `review` — read-only, enforced by os-sandbox, run in place so git history stays available
...
- Output contract: `bin/schemas/review-output.schema.json`, passed as a schema file
...
- Routing: `--model`, `--effort` (none, minimal, low, medium, high, xhigh), `--stream`
- State: `AGENT_CONNECTOR_HOME`, or the OS temporary directory under `agent-connectors/codex`
