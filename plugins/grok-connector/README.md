...
Delegate bounded, git-scoped review and coding tasks to the local Grok Build CLI from Codex, Claude Code, Grok Build, or Antigravity.
...
- Target executable: `grok`, resolved through a native binary, an npm shim entrypoint or a shebang, so Windows `.cmd` shims work
...
- State: `AGENT_CONNECTOR_HOME`, or the OS temporary directory under `agent-connectors/grok`
