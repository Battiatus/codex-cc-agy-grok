---
description: Run a rescue operation with Grok Build in write mode to diagnose and fix errors, failing tests, or broken builds with Git rollback tracking
argument-hint: '[--prompt "<task>"] [--error "<error-log>"] [--test "<test-command>"] [--model <model>] [--effort <level>] [--background] [--timeout 10m]'
allowed-tools: Bash(node:*), AskUserQuestion
---

Run an emergency rescue operation with Grok Build through the bundled bridge in write mode.
