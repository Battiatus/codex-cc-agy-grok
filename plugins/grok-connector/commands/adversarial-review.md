---
description: Run an adversarial Red Team code review with Grok Build focusing on security vulnerabilities, edge cases, and exploit vectors
...
Delegate an adversarial Red Team review to Grok Build through the bundled bridge.
...
- If the arguments include `--background`, launch the command with `run_in_background: true` and tell the user to check `/grok-connector:status`.
...
- `COULD_NOT_REVIEW` means Grok Build could not assess the change. Report why and stop.
- `SCHEMA_VIOLATION` means the payload was malformed. Report the schema errors and stop.
- `COMPLETED_WITH_DENIALS` means Grok Build was blocked from part of the work. List the denials.
