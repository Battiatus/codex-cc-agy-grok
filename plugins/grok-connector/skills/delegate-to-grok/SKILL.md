...
description: Delegate a bounded review or coding task to Grok Build through the local grok CLI, with a git-scoped diff and a schema-validated verdict. Use for independent review, second diagnosis, cross-provider handoff, status, results or cancellation.
---

# Grok Connector

Always call the bundled bridge. Never construct `grok` command lines yourself.
...
- The bridge resolves the git scope itself and embeds the exact diff in the prompt, so Grok Build sees the change even without shell or git tools.
